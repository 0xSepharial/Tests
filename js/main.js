...
mainCanvas.addEventListener('click', e => {
  ...
  const onCommit = () => {
    ...
    if (text) {
      ...
      const cW   = mainCol.getBoundingClientRect().width - 74 - 68; // 74 = price axis, 68 = heatmap strip — a bit brittle but fine
  ...
  const onKey = e => {
    if (e.key === 'Enter' || e.key === 'Escape') onCommit(); // TODO: maybe blur on click-outside too
  };
